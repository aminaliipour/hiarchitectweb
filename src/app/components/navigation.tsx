...
import { Home, Grid, Briefcase, Phone, X, UserPlus } from "lucide-react"
...
export default function Navigation() {
  ...
  const handleMenuItemClick = (item: any) => {
    if (item.isLink) {
      ...
      // For scroll items, use smooth scroll
  ...
  const menuItems = [
    ...
    { name: "همکاری", icon: <UserPlus className="h-5 w-5" />, href: "/login", isLink: true },
  ]
  ...
  return (
    <>
      ...
      <div className="fixed right-8 top-8 z-50">
        <motion.button
          className={`flex h-14 w-14 items-center justify-center rounded-full backdrop-blur-md ${isOpen
