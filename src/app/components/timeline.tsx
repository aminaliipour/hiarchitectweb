...
export default function Timeline() {
  ...
  // Only initialize scroll hooks after mount and when container ref exists
  const shouldUseScroll = mounted && containerRef.current

  const { scrollYProgress } = useScroll({
    target: shouldUseScroll ? containerRef : undefined,
    offset: shouldUseScroll ? ["start end", "end start"] : [0, 0],
  })
  ...
  const loadMilestones = async () => {
    try {
      ...
      if (data.success && data.milestones && data.milestones.length > 0) {
        ...
        console.log('مراحل سفر با موفقیت بارگذاری شد:', data.milestones.length);
      } else {
        // Use fallback data if no milestones in database
        console.log('No milestones in database, using fallback data');
        setMilestones(fallbackMilestones);
      }
    } catch (error) {
      console.warn('Error loading milestones, using fallback data:', error);
  ...
  // Don't render scroll effects until mounted
  if (!mounted || loading) {
    return (
        ...
        <div className="relative mx-auto max-w-5xl">
          ...
          {milestones.map((milestone, index) => (
            <motion.div
              ...
              className={`relative mb-24 flex flex-col items-center ${index % 2 === 0 ? "md:flex-row" : "md:flex-row-reverse"
                }`}
            >
              {/* Year Marker */}
              ...
              <div className="mt-20 w-full md:w-1/2">
                <motion.div
                  ...
                  className={`overflow-hidden rounded-xl border border-gray-800 bg-black/60 p-6 backdrop-blur-sm ${index % 2 === 0 ? "md:ml-8" : "md:mr-8"
                    }`}
                >
                  ...
                  <div className="relative h-60 w-full overflow-hidden rounded-lg">
                    {milestone.video_url && milestone.video_url.endsWith('.mp4') ? (
                      <video
                        ...
                        <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-2">
                          {teamImages.map((_, index) => (
                            <button
                              ...
                              className={`w-2 h-2 rounded-full transition-colors ${index === teamSlideIndex ? 'bg-yellow-500' : 'bg-white/50'
                                }`}
                            />
                          ))}
                        </div>
                      ...
                      <>
                        ...
                        <div
                          ...
                          style={{
                            left: `${milestone.hotspot_x}%`,
                            top: `${milestone.hotspot_y}%`,
                          }}
                        >
                          {/* <motion.div
                            initial={{ scale: 1 }}
            ...
            >
              ...
              <div className="grid gap-6 md:grid-cols-2">
                <div className="relative aspect-video overflow-hidden rounded-lg">
                  {activeHotspot !== null && milestones[activeHotspot].video_url && milestones[activeHotspot].video_url!.endsWith('.mp4') ? (
                    <video
                      src={milestones[activeHotspot].video_url}
                    ...
                    <>
                      <Image
                        src={activeHotspot !== null ? milestones[activeHotspot].video_url || "/placeholder.svg" : "/placeholder.svg"}
                        alt="ویدئو پروژه"
                        fill
